        ...
        <div class="box">
          <div class="box-header with-border">
            <div class="pull-right">
              ...
              <div class="btn-group pull-right" style="margin-right: 10px">
                <a href="{{route('system_admin.category.create')}}" class="btn btn-sm btn-success" title="New">
              ...
              <tbody>
                @if (count($color) >0)
                  @foreach($color as $color)
                    <tr>
                      <td><input type="checkbox" class="grid-row-checkbox" data-id="{{ $color->id }}" /></td>
                      <td>
                        {{ $color->id }}
                      </td>
                      <td>
                        <a class="text-left" href="" title="{{ $color['name'] }}">{{ $color['name'] }}</a>
                      </td>
                      <td> {{ \Carbon\Carbon::parse($color->created_at)->format('d/m/Y')}}</td>
                      <td> {!! \App\Helpers\Common::checkStatus($color->status) !!} </td>
                      <td>
                        <a href="{{route('system_admin.category.edit',['id'=>$color->id])}}" class="btn btn-icon btn-sm btn-primary tip">
                          <i class="fa fa-eye"></i>
                        </a>
                        <a href="javascript:void(0);" data-id="{{ $color->id }}" class="btn btn-icon btn-sm btn-danger deleteDialog tip">
