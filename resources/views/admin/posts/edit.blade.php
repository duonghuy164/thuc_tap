...
@section('content')
  <section class="content">
    ...
    <form method="POST" action="{{ route( 'system_admin.post.update' , ['id'=>$post->id] ) }}">
      @method('PUT')
      {{ csrf_field() }}
      @if ($errors->all())
      ...
      @else 
        <div class="note note-success"><p>Bạn đang chỉnh sửa bài viết</p></div>
      @endif
      <div class="row">
        <div class="col-md-9">
            ...
            <div class="tab-content">
              <div class="tab-pane active show" id="tab_detail">
                <div class="form-group">
                  ...
                  <input class="form-control" placeholder="Tiêu đề bài đăng" data-counter="120" name="title" type="text" id="title" value="{{ $post->title }}">
        ...
        <div class="col-md-3 right-sidebar">
            ...
            <div class="widget-body">
              <div class="ui-select-wrapper">
                <select class="form-control ui-select ui-select" id="status" name="status">
                  <option value="1" @if ( $post->status == 1  ) selected @endif>Đã kích hoạt</option>
                  <option value="0" @if ( $post->status == 0  ) selected @endif>Đã vô hiệu</option>
                </select>
              ...
              <div class="form-group form-group-no-margin ">
                <div class="multi-choices-widget list-item-checkbox">
                  @if(count($categories)>0)
                    {!! \App\Helpers\Common::showCategories($categories,$post->cat_id,0,'cat_id') !!}
                  @endif
                </div>
              </div>
            </div>
          ...
          <div class="widget meta-boxes">
            ...
            <div class="widget-body">
              <div class="image-box">
                @if ($post->thumbnail)
                  <input id="thumbnail" type="hidden" name="thumbnail" value="{{ $post->thumbnail }}" class="image-data">
                  <div class="preview-image-wrapper ">
                    <img id="holder" class="preview_image" src="{{ $post->thumbnail }}" type="text" name="filepath" alt="preview image">
