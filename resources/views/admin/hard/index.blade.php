        ...
        <div class="box">
          <div class="box-header with-border">
            <div class="pull-right">
              ...
              <div class="btn-group pull-right" style="margin-right: 10px">
                <a href="{{route('system_admin.hard.create')}}" class="btn btn-sm btn-success" title="New">
          ...
          <div class="box-body table-responsive no-padding">
            <table class="table table-hover">
              ...
              <tbody>
                @if (count($hard) >0)
                  @foreach($hard as $hard)
                    <tr>
                      <td><input type="checkbox" class="grid-row-checkbox" data-id="{{ $hard->id }}" /></td>
                      <td>
                        {{ $hard->id }}
                      </td>
                      <td>
                        <a class="text-left" href="" title="{{ $hard['name'] }}">{{ $hard['name'] }}</a>
                      </td>
                      <td> {{ \Carbon\Carbon::parse($hard->created_at)->format('d/m/Y')}}</td>
                      <td> {!! \App\Helpers\Common::checkStatus($hard->status) !!} </td>
                      <td>
                        <a href="{{-- {{route('system_admin.cpu.edit',['id'=>$cpu->id])}} --}}" class="btn btn-icon btn-sm btn-primary tip">
                          <i class="fa fa-eye"></i>
                        </a>
                        <a href="javascript:void(0);" data-id="{{ $hard->id }}" class="btn btn-icon btn-sm btn-danger deleteDialog tip">
