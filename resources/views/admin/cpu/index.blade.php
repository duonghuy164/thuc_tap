        ...
        <div class="box">
          <div class="box-header with-border">
            <div class="pull-right">
              ...
              <div class="btn-group pull-right" style="margin-right: 10px">
                <a href="{{route('system_admin.cpu.create')}}" class="btn btn-sm btn-success" title="New">
            ...
            <table class="table table-hover">
              ...
              <tbody>
                @if (count($cpu) >0)
                  @foreach($cpu as $cpu)
                    <tr>
                      <td><input type="checkbox" class="grid-row-checkbox" data-id="{{ $cpu->id }}" /></td>
                      <td>
                        {{ $cpu->id }}
                      </td>
                      <td>
                        <a class="text-left" href="" title="{{ $cpu['name'] }}">{{ $cpu['name'] }}</a>
                      </td>
                      <td> {{ \Carbon\Carbon::parse($cpu->created_at)->format('d/m/Y')}}</td>
                      <td> {!! \App\Helpers\Common::checkStatus($cpu->status) !!} </td>
                      <td>
                        <a href="{{-- {{route('system_admin.cpu.edit',['id'=>$cpu->id])}} --}}" class="btn btn-icon btn-sm btn-primary tip">
                          <i class="fa fa-eye"></i>
                        </a>
                        <a href="javascript:void(0);" data-id="{{ $cpu->id }}" class="btn btn-icon btn-sm btn-danger deleteDialog tip">
